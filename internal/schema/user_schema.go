...
type UserEmailLoginReq struct {
	...
	Pass        string `validate:"required,gte=4,lte=64" json:"pass"`
...
type UserModifyPasswordReq struct {
	OldPass     string `validate:"omitempty,gte=4,lte=32" json:"old_pass"`
	Pass        string `validate:"required,gte=4,lte=32" json:"pass"`
...
type UserChangeEmailSendCodeReq struct {
	...
	Email  string `validate:"required,email,gt=0,lte=500" json:"e_mail"`
	Pass   string `validate:"omitempty,gte=4,lte=32" json:"pass"`
	UserID string `json:"-"`
}
